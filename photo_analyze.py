...
#!/usr/bin/env python3
"""
Photo analysis and reporting pipeline with UUID primary key.
Scans directory, extracts EXIF, performs image tagging, stores results in DuckDB, and creates summary graphs. Each row has a unique UUIDv4 hex id as the primary key.
"""
...
def init_db(conn):
    # id is TEXT PRIMARY KEY using UUID hex generated by the script
    conn.execute(f"""
...
def insert_record(conn, rec):
    # Ensure each record gets a UUID hex id
    row_id = rec.get("id") or uuid.uuid4().hex
    conn.execute("""
    INSERT INTO photos (
...
def scan_and_analyze(root_dir, db_path):
    ...
    for fp in tqdm(files):
        ...
        if season and season not in cats:
            cats.append(season)
